...
const getGeocode = async (location) => {
  const URL = `${MAPBOX_BASE}${location}.json?types=place&access_token=pk.eyJ1IjoiZGF3NTY4IiwiYSI6ImNra2I2Z284YTBiM3Eyb3FvcmV1cnFzcTIifQ.oKrbJWy1DUP0xERsbHGUFA`;
  ...
  const coordinates = geocodeList[0] ? geocodeList[0].center : "";
  ...
  const country = geocodeList[0].context[1]  ? geocodeList[0].context[1].text : "";
...
export const getWeather = async (location) => {
  ...
  const URL = `${WEATHER_BASE}?lat=${lat}&lon=${lon}&exclude=minutely,hourly,alerts&units=metric&appid=422063eedea7b5be7277d07fe5faa275`;
